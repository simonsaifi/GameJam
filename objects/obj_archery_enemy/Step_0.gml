...
if (place_meeting(x, y, obj_shuriken)&& state != "die" && !global.is_transformed) { // Si l'archer se prend un shuriken
...
}  else if (place_meeting(x, y, obj_ninja) && !isAttacking && state != "die") { // Si le joueur est touché par l'ennemi au cac
    ...
    isAttacking = true;
	scr_ninja_get_hit();
}  else if (place_meeting(x, y, obj_attackEffect) && state != "hit" && state != "die") { // Si l'ennemi se prend un coup d'épée
    animation_done = false; // Animation définie sur pas terminée
    image_index = 0; // Réinitialiser l'index de l'image
